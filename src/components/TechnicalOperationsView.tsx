...
import { TechnicalIssue, SupplyRequest, TechnicalActionLog, TechnicalStatusChange, UserRole } from '../types';

interface TechnicalOperationsViewProps {
  selectedCenterId: string;
  onAddLog: (action: string, details: string) => void;
  activeSubView?: 'dashboard' | 'issues' | 'requests' | 'ai-copilot' | 'reports' | 'areas';
  onChangeSubView?: (view: 'dashboard' | 'issues' | 'requests' | 'ai-copilot' | 'reports' | 'areas') => void;
}
...
export default function TechnicalOperationsView({
  ...
  const setSubTab = (tab: 'dashboard' | 'issues' | 'requests' | 'ai-copilot' | 'reports' | 'areas') => {
    if (onChangeSubView) {
      onChangeSubView(tab);
    } else {
  ...
  const getPriorityBadge = (prio: string) => {
    switch (prio) {
      ...
      case 'Orta': return 'bg-blue-100 text-blue-700 border-blue-200';
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'İşleme Alındı': return 'bg-blue-50 text-blue-700 border-blue-200';
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-150 shadow-3xs flex flex-col justify-between">
              ...
              <div className="py-4 flex items-center justify-center">
                <svg viewBox="0 0 200 100" className="w-full max-h-[120px]">
                  ...
                  {/* Bars */}
                  {/* Tesisat */}
                  <rect x="40" y="30" width="16" height="50" fill="#3b82f6" rx="2" />
                  ...
                  {/* Values */}
                  <text x="48" y="25" textAnchor="middle" fontSize="8" fill="#3b82f6" className="font-extrabold">25%</text>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-[10px] font-bold">
                      ...
                      <button
                        ...
                        className="py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg transition cursor-pointer"
                      >
                        İşleme Al (Müdahale)
                      </button>
                      <button
            ...
            <div className="bg-white rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:rounded-none print:w-full print:border-0" id="report-document-modal">
              ...
              <div className="p-8 overflow-y-auto flex-1 space-y-6 print:overflow-visible print:p-0" id="printable-report-document">
                ...
                <div className="border-b-2 border-emerald-800 pb-4 flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-emerald-700 flex items-center justify-center font-extrabold text-white text-xs">Y</div>
                      <span className="text-xs font-black tracking-widest text-emerald-800 uppercase">T.C. YEŞİLAY CEMİYETİ</span>
                    </div>
                    <h2 className="text-sm font-black text-gray-800">{campCenterName.toUpperCase()}</h2>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">KAMPÜS TEKNİK VE OPERASYONEL YÖNETİM SİSTEMİ</p>
                  </div>
